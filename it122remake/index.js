...
app.delete("/api/products/delete/:id", async (req, res, next) => {
    const id = req.params.id;
    const product = await Cosmetic.findByIdAndDelete(id);
    if (!product) {
        res.status(404).json({
            message: "Product not found"
        });
...
app.patch("/api/products/update/:id", async (req, res, next) => {
    const id = req.params.id;
    const product = await Cosmetic.findByIdAndUpdate(id, req.body);
    if (!product) {
